...
import { SocialPostGallery } from "@/components/sections/social-post-gallery";

const INSTAGRAM_GALLERY_LIMIT = 9;
...
export async function FollowSection() {
  ...
  const [facebookPosts, instagramPosts] = await Promise.all([
    getFacebookPosts(1),
    getInstagramPosts(INSTAGRAM_GALLERY_LIMIT),
  ]);
  ...
  return (
    <Section id="seguici" tone="ink" wide crossSize="sm">
      ...
      <div className="mx-auto grid min-w-0 max-w-5xl gap-14 lg:grid-cols-2">
        ...
        <FadeIn delay={0.08} className="min-w-0">
          ...
          <SocialPostGallery
            ...
            noCaptionLabel={t("noCaption")}
            galleryLabel={t("instagramGallery")}
            prevLabel={t("galleryPrev")}
            nextLabel={t("galleryNext")}
          />
          <p className="mt-3 text-sm text-white/55">{t("instagramFeedHint")}</p>
          <a
            href={site.social.instagram}
            target="_blank"
            rel="noopener noreferrer"
